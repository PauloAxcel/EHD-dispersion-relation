...
coef_a = -epsilon0*epsilonp*A
coef_b = -coef_a*(d-3/2*delta)
coef_c = epsilonp-1
coef_d = d*(1-2*epsilonp)+delta*(2*epsilonp-3/2)
coef_e = epsilonp*(d-delta)**2
...
for i in range(space1):
    p[i] = -U**2/(2*A)*(coef_a*(coef_c*h[i]**2+coef_d*h[i]+coef_e)-(coef_a*h[i]+coef_b)*(2*coef_c*h[i]+coef_d))/((coef_c*h[i]**2+coef_d*h[i]+coef_e)**2)
    dp[i] = U**2/(2*A)*((2*coef_c*(coef_a*h[i]+coef_b))/((coef_c*h[i]**2+coef_d*h[i]+coef_e)**2)+(2*(2*coef_c*h[i]+coef_d)*(coef_a*(coef_c*h[i]**2+coef_d*h[i]+coef_e)-((coef_a*h[i]+coef_b)*(2*coef_c*h[i]+coef_d))))/((coef_c*h[i]**2+coef_d*h[i]+coef_e)**3))
