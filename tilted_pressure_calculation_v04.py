...
for i in range(space1):
    ...
    H = epsilonp*Delta[i]**2+h[i]*Delta[i]-h[i]*delta/2
    dH = Delta[i]*(1-2*epsilonp)-h[i]-delta/2
    Q = Delta[i]**2*(1-epsilonp)-Delta[i]*delta*(1-epsilonp)+delta**2/4
    dQ = -2*Delta[i]*(1-epsilonp)+delta*(1-epsilonp)
    
    p[i] = (epsilon0*epsilonp*U**2/2)*(Q/H**2)
    dp[i] = (epsilon0*epsilonp*U**2/2)*((dQ*H-Q*dH)/H**4)
