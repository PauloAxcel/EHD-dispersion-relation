...
delta = np.linspace(0,8E-7,space3)
h = np.linspace(1E-6,2E-6,space1)
...
for k in range(space3):     
    plt.figure(figsize=(9,9/1.618))
    for i in range(space1):
        p[i] = -0.5*(epsilon0*epsilonp*(epsilonp-1)*U**2)/(epsilonp*(d-delta[k]/2)-(epsilonp-1)*h[i])**2
        dp[i] = -(epsilon0*epsilonp*(epsilonp-1)**2*U**2)/(epsilonp*(d-delta[k]/2)-(epsilonp-1)*h[i])**3
...
#        ax3.set_title('delta = '+str(delta[k]))
#    
